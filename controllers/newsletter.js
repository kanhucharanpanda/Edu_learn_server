...
export const unsubscribe = TryCatch(async (req, res) => {
  ...
  // Find the subscriber by email and delete them
  const subscriber = await Newsletter.findOneAndDelete({ email });

  if (!subscriber) {
    // If the email wasn't found, it's good practice to still send a neutral success message
    // This prevents people from checking which emails are in your database.
    return res.json({
      message: "If your email was in our list, it has been removed.",
    });
  }
